...
export async function registrarAssinaturaProposta(
  token: string,
  nomeDigitado: string,
  ...
  const { data, error } = await supabase.rpc("carta_proposta_assinatura_registrar", {
    p_token: token,
  ...
  if (error || data !== true) {
    return { ok: false, erro: "Não foi possível registrar a assinatura. O link pode já ter sido usado." };
  }

  revalidatePath(`/assinar-proposta/${token}`);
  return { ok: true };
}
